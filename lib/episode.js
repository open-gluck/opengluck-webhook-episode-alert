...
async function handlePhaseOut({ lastEpisode, lastTimestamp, newTimestamp }) {
  ...
  if (lastEpisode === "disconnected") {
    ...
    if (currentHasRealTime) {
      console.log(
        "Skip sending End of Low alert because current device has real-time CGM data"
      );
      return;
    }
    notification.alert = {
    ...
    if (currentHasRealTime) {
      console.log(
        "Skip sending End of High alert because current device has real-time CGM data"
      );
      return;
    }
    notification.alert = {
